**Last Updated: December 31, 2025**

# Staex Case Study: Comprehensive Analysis (December 2025)

## Executive Summary

Staex is a Potsdam-based deep tech company building decentralized infrastructure for Physical AI and the machine economy[^1]. Originally developed at Deutsche Telekom Innovation Labs (T-Labs)[^2], Staex was acquired through a management buyout in 2021[^3] and has since established itself as a leading provider of secure, zero-trust networking for IoT devices, Physical AI systems, and Decentralized Physical Infrastructure Networks (DePIN)[^4]. The platform enables machines to communicate securely, produce verifiable data, and execute autonomous transactions without cloud dependencies[^5].

With strategic partnerships spanning major blockchain ecosystems including peaq[^6], Internet Computer (ICP)[^7], Lisk[^8], Solana[^9], U2U Network[^10], and Linera[^11], Staex is positioning itself at the intersection of industrial IoT and Web3 infrastructure[^12]. The company led the NEXA consortium that deployed the world's first Web3-operated drone corridor in Berlin in May 2024[^13], winning the Berlin Deep Tech Award 2024 in the Web3 category[^14].

---

## Key Information (December 2025)

| Metric | Value | Notes |
|--------|-------|-------|
| **Company Stage** | Seed / Pre-Series A | [^15] |
| **Total Funding** | €1.65M+ (pre-seed confirmed) | [^16] |
| **Founded** | 2021 | [^17] |
| **Headquarters** | Potsdam, Germany | [^18] |
| **Origin** | Deutsche Telekom T-Labs management buyout | [^19] |
| **Token Status** | No token announced | [^20] |
| **Category** | Physical AI / DePIN / IoT Infrastructure | [^21] |
| **Key Recognition** | Berlin Deep Tech Award 2024 | [^22] |
| **Webthreepedia Report** | [Staex Analysis](https://webthreepedia.com) | [^22a] |

**Note**: Staex is a seed-stage B2B infrastructure company. Unlike DeFi protocols, there is no TVL, token market cap, or on-chain trading metrics to report. Data sources are primarily news, partnerships, and official announcements.

---

## Company Overview

### Background & Origin

Staex's technology development began in 2017 at Deutsche Telekom Innovation Laboratories (T-Labs)[^23], the R&D arm of Europe's largest telecommunications company. The founding team spent four years developing secure, blockchain-integrated networking solutions for IoT devices before executing a management buyout in 2021 to form Staex GmbH[^24].

The company is headquartered in Potsdam, Germany[^25]. Previously, Staex operated from The Drivery co-working space in Berlin, a mobility-focused innovation hub that has since closed[^25a]. This origin story provides Staex with unique credibility—the core technology was validated within one of Europe's largest telcos before becoming an independent venture through management buyout[^26].

### Vision: The Machine Economy

Staex addresses a fundamental challenge: enabling the projected **75 billion machine connections by 2030**[^27] to communicate securely, exchange value, and coordinate autonomously. The company's vision centers on machines that can[^28]:

- Produce auditable, tamper-proof data signals[^29]
- Align on shared truths for multi-machine coordination[^30]
- Make autonomous decisions even in unstable connectivity scenarios[^31]
- Execute value transactions without human intermediation[^32]

CEO Dr. Alexandra Mikityuk states: "Connecting machines with a plethora of blockchain and DLT protocols will enable a global machine-to-machine economy, where machines will be able to securely execute value transactions."[^33]

---

## Leadership Team

### Dr. Alexandra Mikityuk - CEO & Co-Founder

Dr. Mikityuk brings deep expertise in cryptography and distributed systems[^34]:

| Background | Details |
|------------|---------|
| **Education** | PhD from TU Berlin[^35] |
| **Academic Role** | Professor for Computer Science of Communication Technology at HTW Berlin[^36] |
| **Deutsche Telekom** | Head of Blockchain Group at T-Labs (2017-2021)[^37] |
| **Focus Areas** | Decentralized systems, blockchain, security, Physical AI[^38] |

### Philip Toepffer - CFO & Co-Founder

Philip Toepffer leads finance and business development[^39]:

| Background | Details |
|------------|---------|
| **Education** | External PhD candidate at TU Berlin[^40] |
| **Expertise** | Finance, business development, blockchain[^41] |
| **Role** | Originally COO, now CFO[^42] |

### Paksy Plackis-Cheng - CSO & Co-Founder

Paksy brings global strategy and policy expertise[^43]:

| Background | Details |
|------------|---------|
| **Experience** | 15 years across startups, corporates, public sector in 20+ countries[^44] |
| **Academic** | Former Senior Fellow at UC Santa Barbara[^45] |
| **Policy Work** | EU AI policy advisor (Fairness and Intersectional Non-Discrimination initiative)[^46] |
| **Prior Work** | Supported $6.9B IPO-stage startup[^47] |
| **Other Ventures** | Founder of impactmania[^48] |

---

## Funding History

**Total Funding**: $1.81M+ across multiple rounds[^49]

### Pre-Seed Round (April 2022)

Staex raised €1.65 million in its pre-seed round[^49a]:

| Investor | Type | Notes |
|----------|------|-------|
| **Brandenburg Kapital** | Lead investor | Subsidiary of Investitionsbank des Landes Brandenburg[^50] |
| **Blockwall Capital** | Co-lead | European blockchain-focused VC[^51] |
| **Andreas Möller** | Angel investor | Experienced business angel[^52] |

The funding was designated for product development and team expansion to bring Staex's Web3 stack for IoT to market[^53].

### Follow-on Investments (2024-2025)

#### Moonrock Capital (April 2024)

Moonrock Capital invested in Staex with ongoing support from Brandenburg Kapital[^54]:
- Recognized "the need of a decentralized network realizing machine-to-machine economy"[^54a]
- Focus on clean-tech software enabling distributed IT networks[^54b]

#### The Drivery (2024)

The Drivery GmbH, Europe's largest marketplace for mobility innovators, made an early-stage investment[^55]:
- Reflects belief in Staex's innovative approach to IoT management[^55a]
- Sebastian Leber, Director of Innovation Programs: "We see strong market validation through numerous projects"[^55b]

#### MN Capital (2024)

MN Capital, a VC firm founded by crypto analyst Michaël van de Poppe, invested to support[^56]:
- Development of network solutions for mobility and energy sectors[^56a]
- Building the first DePIN public transport network[^56b]

#### Generative Ventures (January 2025)

Generative Ventures, the early-stage venture firm specializing in the machine economy, announced its investment[^57]:
- Lex Sokolin, Partner at GV: "Staex is tackling one of the most significant challenges in the evolution of the internet"[^57a]
- Partnership aims to address scalability and security challenges in DePIN[^57b]

### Notable Angel Investors

| Investor | Background |
|----------|------------|
| **EV3 Labs** | Affiliate of Escape Velocity (EV3) focusing on DePIN development, including AR, AI, and IoT solutions. Invests via EV3 Ventures; builds community tools like DePIN Ninja analytics and DePIN Summit[^58] |
| **Sean Carey** | Co-founder and former CTO of Helium Systems and BlockJoy; 25+ years in tech; Lead Partner at Borderless Capital focusing on DePIN investments; deep expertise in distributed systems and scalability[^58a] |
| **Christoph Jentzsch** | Theoretical physicist; former core developer of Ethereum (2014/15); co-founder of slock.it and author of TheDAO; CEO of Tokenize.it; Capital Magazine's Top 40 Under 40; Best Regulatory Innovation in Blockchain 2024[^58b] |

**Source for angel investors**: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025)[^58c]

---

## Technology Architecture

### Three-Layer Stack

Staex's production-ready connectivity stack is structured in three layers[^61]:

#### 1. Connectivity Layer

| Feature | Description |
|---------|-------------|
| **SIM Support** | EU SIM/eSIM for flexible connectivity[^62] |
| **Roaming** | Multi-network roaming capabilities[^63] |
| **Redundancy** | Multi-network redundancy for reliability[^64] |
| **Multi-hop** | Unique multi-hop architecture for complex topologies[^65] |

#### 2. Security & Identity Layer

| Feature | Description |
|---------|-------------|
| **Zero-Trust** | All traffic blocked by default unless specified[^66] |
| **Blockchain Identity** | Integration with peaq IDs, ICP[^67] |
| **P2P Communication** | Encrypted peer-to-peer tunnels[^68] |
| **Tamper-Proof Logging** | Verifiable, immutable event records[^69] |
| **On-Device Signing** | Private keys remain on device[^70] |

#### 3. Smart Billing Layer

| Feature | Description |
|---------|-------------|
| **Usage Tracking** | Granular metering of connectivity[^71] |
| **Automated Policies** | Rule-based billing automation[^72] |
| **Transparent Costs** | Clear cost management[^73] |
| **On-Chain Invoicing** | Blockchain-based invoice settlement[^74] |

### Key Technical Differentiators

1. **Legacy Protocol Encryption**: Secures outdated IoT protocols that can't be updated[^75]
2. **5x Lower Footprint**: More lightweight than Kubernetes alternatives[^76]
3. **VPN Alternative**: Works with devices that can't run VPNs (smart meters, IP cameras, EV chargers)[^77]
4. **Master-less Architecture**: No central point of failure[^78]
5. **Privacy-First**: Only public keys stored on-chain; private keys stay with device owners[^79]

---

## Blockchain Integrations

### peaq Network

Staex joined peaq as a networking and indexing layer for DePIN in January 2024[^80]:

| Component | Function |
|-----------|----------|
| **Indexing Pallet** | Track devices connecting to peaq network[^81] |
| **Mesh Communication Component (MCC)** | Service discovery and tunneling between devices[^82] |
| **peaq IDs** | Blockchain-based device identity[^83] |
| **Role-Based Access Control** | Permission management via peaq access[^84] |

Staex completed two milestones in peaq's DePIN Grant Program[^85], delivering CLI tools for builders to leverage these integrations[^86].

### Internet Computer (ICP)

Staex built the Vehicle Tracking System (VTS) on ICP[^87]:

| Feature | Implementation |
|---------|----------------|
| **Smart Contracts** | Rust SDK with Stable Structures library[^88] |
| **Signature Verification** | On-chain verification using k256 crypto library[^89] |
| **Automated Invoicing** | Canister executes monthly to issue invoices[^90] |
| **Access Control** | Vehicles unlocked/blocked based on payment status[^91] |

Key advantages of ICP integration[^92]:
- Large on-chain data storage capacity
- Reverse gas model simplifies device onboarding
- Digital signature verification enables trusted telemetry

### Lisk Blockchain

Staex partnered with Lisk for the RISE (dRone InfraStructure paymEnts) project[^93]:

- Joined Lisk Accelerator in August 2023[^94]
- NEXA drone corridor stores video hashes on Lisk blockchain for authenticity verification[^95]
- Working toward Lisk mainnet integration[^96]

### Solana (via Superteam Germany)

Announced July 2025, Staex partnered with Solana Superteam Germany to scale DePIN solutions[^97]:

- Leveraging Solana's infrastructure for real-time data sharing[^98]
- Focus on transparency and decentralized infrastructure in Germany[^99]

### U2U Network

U2U integrated Staex for DePIN device coordination[^100]:

- Secure peer-to-peer communication for IoT devices[^101]
- Cloud-free, verifiable data exchange[^102]
- Expansion across DeFi, RWA, and DePIN verticals[^103]

### Linera

Staex partnered with Linera to test high-frequency telemetry on microchains[^104]:

| Phase | Details |
|-------|---------|
| **Pilot** | 10 city buses in North Rhine-Westphalia, Germany[^105] |
| **Capacity** | Up to 1 million transactions per day[^106] |
| **Data Types** | Performance metrics, location, diagnostics[^107] |
| **Next Phase** | 100+ buses in production deployment[^108] |

CSO Paksy Plackis-Cheng: "Linera's microchain architecture provides Staex with the parallelism and reliability required to manage thousands of critical edge devices in near real time."[^109]

---

## Major Projects & Deployments

### NEXA Consortium (May 2024)

NEXA is a pioneering public-private partnership demonstrating machine-to-machine economy capabilities[^110]:

| Detail | Information |
|--------|-------------|
| **Led by** | Staex GmbH[^111] |
| **Launch Date** | May 21, 2024 (Berlin Blockchain Week)[^112] |
| **Location** | Marienpark, Berlin[^113] |
| **Inaugurated by** | Dr. Severin Fischer, German Secretary of State[^114] |

**NEXA Partners** (9 organizations)[^115]:
1. Vantage Towers - Europe's leading tower operator (84,000+ sites)[^116]
2. paymenttools (REWE Group) - Payment infrastructure[^117]
3. Berlin Partner - Economic development agency[^118]
4. Chirp Wireless - DePIN network (50+ countries)[^119]
5. Lisk - Blockchain infrastructure[^120]
6. Internet Computer (ICP) - Decentralized compute[^121]
7. Germandrones - Drone hardware[^122]
8. The Drivery - Mobility innovation hub[^123]
9. Marienpark - Launch venue[^124]

**Achievement**: World's first Web3-operated drone corridor[^125]. During the demonstration, a Songbird drone filmed the event, with the video instantly available for purchase and authenticity verified via Lisk blockchain[^126].

**Recognition**: Berlin Deep Tech Award 2024 (Web3 category)[^127], selected from 100 applicants and 15 finalists[^128]. Award endowed with €10,000[^129].

### MINT Project

The MINT project brings trusted telemetry to public transport[^130]:

| Partner | Contribution |
|---------|--------------|
| **MIBO** | 150-bus fleet for pilot[^131] |
| **Solana** | Blockchain infrastructure[^132] |
| **U2U Network** | DePIN platform[^133] |
| **DIMO** | AI modeling of vehicle data[^134] |
| **IRIS** | Passenger counting and video security[^135] |

**Technical Implementation**[^136]:
- Data directly signed by vehicles ensuring source authenticity
- Metadata (passenger count, CO2 values) stored on decentralized ledger
- Immutable, manipulation-proof record keeping

### PANTER Platform

PANTER enables new business models for transport operators[^137]:

- Real-time data capture from vehicles and infrastructure[^138]
- Secure machine-to-machine transactions[^139]
- Automated invoicing, auditing, and certification[^140]
- Revenue stream unlocking for public and commercial transport[^141]

### Chirp Partnership (200,000 Buses)

Staex partnered with Chirp for one of the largest decentralized infrastructure deployments globally[^142]:

| Metric | Value |
|--------|-------|
| **Target Scale** | Up to 200,000 municipal buses[^143] |
| **Devices per Vehicle** | Up to 12 IoT sensors[^144] |
| **Data Collected** | Air quality, temperature, humidity, movement[^145] |
| **Chirp Network** | 50+ countries, 476+ Blackbird Miners, 1.2M+ IoT devices[^146] |

### Vehicle Tracking System (VTS)

Built on Internet Computer, VTS provides[^147]:

- Trusted, transparent vehicle usage infrastructure[^148]
- Decentralized telemetry signing and storage[^149]
- On-chain contract agreements between parties[^150]
- Automated invoice generation via ICP canisters[^151]
- Remote vehicle functionality control based on payment status[^152]

**Current Status**: Proof of concept complete, proceeding to hardware integration and customer testing[^153].

---

## Strategic Partnerships

### Vantage Towers

Vantage Towers is Europe's leading telecommunications tower company[^154]:

| Metric | Value |
|--------|-------|
| **Tower Sites** | 84,600 across 10 European countries[^155] |
| **Key Markets** | Germany, Spain, Greece[^156] |
| **Use Case** | Drone corridors using tower infrastructure for vertiports[^157] |

Vantage Towers uses Staex to securely collect and verify telemetry data from infrastructure[^158], exploring towers as platforms for commercial drone operations[^159].

### Tower Automation Alliance (TAA)

Staex joined TAA to deliver secure data infrastructure for the telco industry[^160]. The partnership builds on two years of successful collaboration on the Web3 drone corridor initiative for Vantage Towers[^161].

### Germandrones

Strategic partnership for drone integration[^162]:

- Hardware provider for NEXA drone corridor[^163]
- Songbird drone used in Berlin demonstration[^164]

---

## Target Markets & Use Cases

### Primary Verticals

Staex targets sectors requiring secure, verifiable machine communication[^165]:

1. **Autonomous Mobility**: Self-driving vehicles, delivery robots[^166]
2. **Industrial Networks**: Factory automation, IIoT[^167]
3. **Public Transport**: Buses, trams, rail systems[^168]
4. **Logistics**: Fleet management, supply chain[^169]
5. **Energy**: Smart grids, EV charging infrastructure[^170]
6. **Physical AI**: AI systems interfacing with real-world machines[^171]

### Geographic Focus

- **Primary**: Germany and Europe (NEXA, Vantage Towers, Linera pilot)[^172]
- **Expansion**: Latin America (Bus2/UnB partnership for Brasilia demo in October 2025)[^173]
- **Global**: Via blockchain partnerships (Solana, ICP, peaq ecosystems)[^174]

---

## Competitive Positioning

### Market Context: DePIN

The Decentralized Physical Infrastructure Network (DePIN) sector addresses real-world connectivity and data challenges[^175]:

- **Problem**: Traditional IoT relies on cloud providers, creating centralization, lock-in, and trust issues[^176]
- **Opportunity**: 75 billion connected machines by 2030 need secure infrastructure[^177]
- **Staex Position**: Enterprise-grade, telco-heritage solution bridging Web2 and Web3[^178]

### Competitive Advantages

| Advantage | Description |
|-----------|-------------|
| **Deutsche Telekom Heritage** | Technology proven in major telco R&D before spinout[^179] |
| **Zero-Trust Architecture** | Security-first design, not bolted on afterward[^180] |
| **B2B Focus** | Enterprise customers (Vantage Towers), not retail[^181] |
| **Legacy Protocol Support** | Can secure devices that can't be updated[^182] |
| **Multi-Blockchain** | Integrations across peaq, ICP, Lisk, Solana, U2U, Linera[^183] |

### Technical Moat

1. **Patented Technology**: Cutting-edge technology from Deutsche Telekom era[^184]
2. **Multi-hop Mesh**: Unique architecture for complex network topologies[^185]
3. **5x Efficiency**: Lower software footprint than alternatives[^186]

---

## Investment Considerations

### Strengths

- **Strong Origin Story**: Deutsche Telekom T-Labs validation[^187]
- **Award-Winning**: Berlin Deep Tech Award 2024 (selected from 100 applicants)[^188]
- **Real Deployments**: NEXA drone corridor, Vantage Towers, MIBO buses[^189]
- **Enterprise Partnerships**: Tier-1 telco tower operator, payment providers[^190]
- **Multi-Chain Strategy**: Not dependent on single blockchain ecosystem[^191]
- **Experienced Team**: PhD leadership, cryptography expertise, policy connections[^192]

### Risks & Considerations

| Risk | Details |
|------|---------|
| **Early Stage** | Pre-Series A, limited funding runway[^193] |
| **No Token** | No public market for liquidity[^194] |
| **B2B Sales Cycle** | Enterprise deals take longer than retail adoption[^195] |
| **Competition** | Other DePIN infrastructure providers[^196] |
| **Execution** | Scaling from pilots to production deployments[^197] |

### Milestones to Watch

1. **Series A Funding**: Will validate market traction and extend runway[^198]
2. **Token Launch**: If announced, creates public market access[^199]
3. **MINT Production**: Moving from 150-bus pilot to scale deployment[^200]
4. **Linera Production**: 100+ bus deployment following pilot[^201]
5. **Chirp Scale**: Progress toward 200,000 bus target[^202]
6. **Brasilia Demo**: Latin America expansion (October 2025)[^203]

---

## Community & Social Presence

### Current Status

As of December 2025, Staex is in early community-building phase[^204]:

| Platform | Status |
|----------|--------|
| **Twitter** | @Staex_io - Active with partnership announcements[^205] |
| **Discord** | Newly opened, <100 members[^206] |
| **LinkedIn** | Company page with team updates[^207] |
| **GitHub** | github.com/staex-io with VTS and other projects[^208] |

### Key Messaging Themes

From official Staex communications[^209]:

- **Physical AI Scalability**: Verifiable real-world data signals for AI systems[^210]
- **Machine Economy**: Enabling autonomous machine transactions[^211]
- **Data Integrity**: Tamper-proof logging and verification[^212]
- **No Cloud Lock-in**: Direct, peer-to-peer connectivity[^213]

---

## Regulatory & Compliance Context

### German & EU Positioning

Staex operates in a relatively favorable regulatory environment[^214]:

- **German Government Support**: NEXA inaugurated by Secretary of State[^215]
- **Berlin Partner Recognition**: Lighthouse project designation[^216]
- **EU Deep Tech Focus**: Aligned with European digital sovereignty goals[^217]

### Potential Regulatory Considerations

| Area | Status |
|------|--------|
| **Data Privacy** | Zero-trust, privacy-first architecture aligns with GDPR[^218] |
| **Drone Regulations** | Working with authorities on permanent corridors[^219] |
| **Token Regulation** | No token announced; MiCA compliance if launched[^220] |

---

## Data Sources & Verification

### Source Categories

This report relies on the following source types (Staex has no API data available)[^221]:

| Category | Examples | Reliability |
|----------|----------|-------------|
| **Official Website** | staex.io, blog posts | High |
| **Press Releases** | Partnership announcements | High |
| **News Coverage** | EU-Startups, CoinDesk, Crypto Daily | Medium-High |
| **Conference Presentations** | Deep Tech Award, Paris Blockchain Week | High |
| **Partner Announcements** | peaq, Lisk, Linera blogs | High |
| **Developer Forums** | DFINITY forum (VTS) | High |

### Limitations

- No on-chain TVL or trading metrics (B2B infrastructure company)[^222]
- No token data (token not announced)[^223]
- Funding amounts beyond pre-seed are undisclosed[^224]
- Some project metrics are forward-looking (e.g., 200,000 buses target)[^225]

---

## Conclusion

Staex represents a compelling play on DePIN infrastructure with unique advantages from its Deutsche Telekom heritage and multi-blockchain integration strategy[^226]. While still at seed stage with limited public funding data, the company has demonstrated execution capability through the NEXA drone corridor, strategic partnerships with Vantage Towers and major blockchain ecosystems, and recognition from German government and deep tech award programs[^227].

Key differentiators include the zero-trust security architecture, ability to secure legacy IoT protocols, and enterprise B2B focus rather than retail speculation[^228]. Investors and ecosystem participants should monitor the company's progression from pilot deployments (MINT, Linera) to production scale, Series A funding timeline, and potential token launch decisions[^229].

The machine economy thesis—75 billion connected devices requiring secure, verifiable communication by 2030—provides a substantial TAM if Staex can capture meaningful market share in enterprise DePIN infrastructure[^230].

---

## Footnotes

[^1]: [Staex Official Website](https://staex.io) - "Staex is the decentralized network layer that securely connects machines and devices while ensuring trusted, verifiable data."

[^2]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "The disruptive team was ex-cubated out of the innovation lab of Deutsche Telekom."

[^3]: [Humboldt Innovation](https://humboldt-innovation.de/en/startupservice/startup-portfolio/staex-gmbh) - Company founded in 2021.

[^4]: [peaq Blog](https://www.peaq.xyz/blog/staex-joins-the-peaqosystem) - "Staex, a secure peer-to-peer networking and software orchestration platform for DePIN."

[^5]: [Staex Official](https://staex.io) - "No middlemen. No cloud lock-ins. Just direct, tamper-proof connectivity."

[^6]: [Chainwire](https://chainwire.org/2024/01/25/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin/) - Staex joins peaq as networking and indexing layer.

[^7]: [DFINITY Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - Staex VTS built on Internet Computer.

[^8]: [Staex Blog](https://staex.io/blog/partnership-lisk) - "Staex and Lisk join forces for Drone Infrastructure Payment Innovation."

[^9]: [Staex Post](https://www.staex.io/post/staex-solana-superteam-germany-partnership) - Partnership with Solana Superteam Germany announced July 2025.

[^10]: [Messari](https://messari.io/report/state-of-u2u-q3-2025) - "U2U integrated with Staex, a protocol that securely connects and coordinates IoT and DePIN devices."

[^11]: [Linera News](https://linera.io/news/linera-x-staex) - Linera partners with Staex for real-world data validation.

[^12]: [Berlin Deep Tech Interview](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - Staex as bridge between Web2 and Web3 technologies.

[^13]: [Staex NEXA Page](https://www.staex.io/nexa) - "World's first Web3-operated drone corridor."

[^14]: [Humboldt Innovation](https://humboldt-innovation.de/en/news/staex-wins-deep-tech-award-again) - "Staex wins Deep Tech Award again."

[^15]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Pre-seed funding stage.

[^16]: [Brandenburg Kapital](https://www.brandenburg-kapital.de/en/bk/news/newsdetailseite_1979904.html) - "€1.65 million in a pre-seed round."

[^17]: [Crunchbase](https://www.crunchbase.com/person/alexandra-mikityuk) - Founded 2021.

[^18]: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025) - Headquarters confirmed as Potsdam, Germany.

[^19]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Staex originated as a management buyout from Deutsche Telekom."

[^20]: Social media analysis - No token announcements as of December 2025.

[^21]: [Breaking DePIN 2025](https://www.breakingdepin.ch/schedule.html) - Staex presenting "Trusted Data Networks: Building the Backbone of DePIN and Physical AI" at Physical AI UnConference, Zurich, September 2025. Physical AI category confirmed via interview with Staex CEO by Maze2 SA CEO.

[^22]: [Berlin Deep Tech Winners](https://www.berlin.de/deeptech/en/blog/artikel.1466383.en.php) - Deep Tech Award 2024 winner in Web3 category.

[^22a]: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025) - Report published on webthreepedia.com.

[^23]: [Staex VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Staex's technology development started in 2017 in Telekom Innovation Laboratories."

[^24]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Management buyout from Deutsche Telekom.

[^25]: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025) - Headquarters confirmed as Potsdam, Germany.

[^25a]: Interview with Staex CEO by Maze2 SA CEO (December 2025) - The Drivery co-working space in Berlin has closed; Staex was always headquartered in Potsdam but previously operated from The Drivery.

[^26]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "Product is based on a patented cutting-edge technology." Management buyout confirmed.

[^27]: [Chainwire](https://chainwire.org/2024/01/25/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin/) - "75 billion machine connections by 2030."

[^28]: [Staex Official](https://staex.io) - Machine economy vision.

[^29]: [Staex Official](https://staex.io) - "Trusted, verifiable data."

[^30]: [Staex Overview](https://staex.io) - "Machines can align on shared truths for coordination."

[^31]: [Staex Technology](https://staex.io) - Autonomous decision-making capability.

[^32]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Machine-to-machine value transactions.

[^33]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Quote from Dr. Alexandra Mikityuk.

[^34]: [Paris Blockchain Week](https://tickets.parisblockchainweek.com/2025/speaker/1651689/alexandra-mikityuk) - Speaker profile.

[^35]: [Best of Blockchain](https://bestofblockchain.io/en/team/dr-alexandra-mikityuk-2/) - "PhD at TU Berlin."

[^36]: [HTW Berlin Faculty Page](https://www.htw-berlin.de/hochschule/personen/person/?eid=15015) - Dr.-Ing. Alexandra Mikityuk, Professor for Computer Science of Communication Technology, Faculty of Engineering Sciences - Energy and Information. Research focus: Security, Blockchain and Web3.

[^37]: [AI Berlin](https://ai-berlin.com/blog/article/paksy-plackis-cheng-founder-impactmania-chief-strategy-officer-co-founder-staex) - "Led the Blockchain Group at Deutsche Telekom Innovation Labs."

[^38]: [Paris Blockchain Week](https://tickets.parisblockchainweek.com/2025/speaker/1651689/alexandra-mikityuk) - Expertise in decentralized systems.

[^39]: [LinkedIn](https://www.linkedin.com/in/philip-toepffer/) - Philip Toepffer profile.

[^40]: [F6S Profile](https://www.f6s.com/member/philip-toepffer) - "External PhD Candidate at TU Berlin."

[^41]: [Humboldt Innovation](https://humboldt-innovation.de/en/startupservice/startup-portfolio/staex-gmbh) - Finance and business development expertise.

[^42]: [The Org](https://theorg.com/org/staex/org-chart/philip-c-toepffer) - Role evolution from COO to CFO.

[^43]: [LinkedIn](https://www.linkedin.com/in/paksy/) - Paksy Plackis-Cheng profile.

[^44]: [AI Berlin](https://ai-berlin.com/blog/article/paksy-plackis-cheng-founder-impactmania-chief-strategy-officer-co-founder-staex) - "15 years of global experience in 20+ countries."

[^45]: [impactmania](https://www.impactmania.com/im/about/) - "Former Senior Fellow at UCSB."

[^46]: [ResearchGate](https://www.researchgate.net/profile/Paksy-Plackis-Cheng) - EU AI policy work.

[^47]: [AI Berlin](https://ai-berlin.com/blog/article/paksy-plackis-cheng-founder-impactmania-chief-strategy-officer-co-founder-staex) - "$6.9B IPO-stage startup" experience.

[^48]: [impactmania](https://www.impactmania.com/im/about/) - Founder of impactmania.

[^49]: [PitchBook Staex Profile](https://pitchbook.com/profiles/company/495000-46) - Total funding $1.81M across multiple rounds.

[^49a]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "€1.65 million pre-seed funding."

[^50]: [Brandenburg Kapital](https://www.brandenburg-kapital.de/en/bk/news/newsdetailseite_1979904.html) - Lead investor details.

[^51]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Blockwall Capital as co-lead.

[^52]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "Including Andreas Möller."

[^53]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Use of funds for product development.

[^54]: [Brandenburg Kapital - Moonrock Capital Investment](https://www.brandenburg-kapital.de/en/bk/news/newsdetailseite_119.html) - Moonrock Capital investment in Staex (April 2024).

[^54a]: [Brandenburg Kapital](https://www.brandenburg-kapital.de/en/bk/news/newsdetailseite_119.html) - "The need of a decentralized network realizing machine-to-machine economy."

[^54b]: [Brandenburg Kapital](https://www.brandenburg-kapital.de/en/bk/news/newsdetailseite_119.html) - Clean-tech software enabling distributed IT networks.

[^55]: [Staex News - The Drivery Investment](https://staex.io/news/the-drivery-invests-in-staex-supporting-future-mobility-innovations) - The Drivery GmbH early-stage investment announcement.

[^55a]: [Staex News](https://staex.io/news/the-drivery-invests-in-staex-supporting-future-mobility-innovations) - "Reflects their belief in Staex's innovative approach."

[^55b]: [Staex News](https://staex.io/news/the-drivery-invests-in-staex-supporting-future-mobility-innovations) - Quote from Sebastian Leber, Director of Innovation Programs.

[^56]: [Staex Post - MN Capital](https://www.staex.io/post/staex-receives-investment-from-mn-capital) - MN Capital investment announcement.

[^56a]: [Staex Post](https://www.staex.io/post/staex-receives-investment-from-mn-capital) - "Mobility and energy sectors."

[^56b]: [Staex Post](https://www.staex.io/post/staex-receives-investment-from-mn-capital) - "First DePIN public transport network."

[^57]: [Staex Post - Generative Ventures](https://www.staex.io/post/https-staex-io-blog-staex-generative-ventures-investment) - Generative Ventures investment announcement (January 2025).

[^57a]: [Staex Post](https://www.staex.io/post/https-staex-io-blog-staex-generative-ventures-investment) - Quote from Lex Sokolin, Partner at Generative Ventures.

[^57b]: [Staex Post](https://www.staex.io/post/https-staex-io-blog-staex-generative-ventures-investment) - Partnership focus on scalability and security in DePIN.

[^58]: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025) - EV3 Labs investor profile: DePIN development, AR, AI, IoT solutions; DePIN Ninja analytics; DePIN Summit.

[^58a]: Interview with Staex CEO by Maze2 SA CEO (December 2025) - Sean Carey investor profile: Co-founder/CTO of Helium Systems and BlockJoy; Lead Partner at Borderless Capital.

[^58b]: Interview with Staex CEO by Maze2 SA CEO (December 2025) - Christoph Jentzsch investor profile: Ethereum core developer (2014/15); co-founder slock.it; author of TheDAO; CEO Tokenize.it; Best Regulatory Innovation in Blockchain 2024.

[^58c]: Interview with Staex CEO Dr. Alexandra Mikityuk by Maze2 SA CEO (December 2025) - Angel investor information provided directly by CEO.

[^61]: [Staex Official](https://staex.io) - Three-layer architecture description.

[^62]: [Staex Technology](https://staex.io) - EU SIM/eSIM support.

[^63]: [Staex Technology](https://staex.io) - Multi-network roaming.

[^64]: [Staex Technology](https://staex.io) - Redundancy features.

[^65]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Multi-hop architecture for complex topologies."

[^66]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Block all network traffic by default."

[^67]: [peaq Blog](https://www.peaq.network/blog/staex-adds-peaq-ids-and-role-based-access-control-to-its-network) - peaq IDs integration.

[^68]: [Staex Official](https://staex.io) - "Peer-to-peer secure communication."

[^69]: [Staex Technology](https://staex.io) - "Tamper-proof logging."

[^70]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Clients retain private keys."

[^71]: [Staex Technology](https://staex.io) - "Granular usage tracking."

[^72]: [Staex Technology](https://staex.io) - "Automated policies."

[^73]: [Staex Technology](https://staex.io) - "Transparent cost management."

[^74]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - On-chain invoicing capability.

[^75]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Encryption of outdated communication protocols."

[^76]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "5 times lower software footprint."

[^77]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "IoT devices that can not run a VPN."

[^78]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "Master-less management."

[^79]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - "Store only public keys in the blockchain."

[^80]: [Chainwire](https://chainwire.org/2024/01/25/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin/) - January 2024 peaq integration.

[^81]: [peaq Blog](https://www.peaq.xyz/blog/staex-joins-the-peaqosystem) - "Indexing pallet for peaq."

[^82]: [Crypto Daily](https://cryptodaily.co.uk/2024/01/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin) - "Mesh Communication Component (MCC)."

[^83]: [peaq Blog](https://www.peaq.network/blog/staex-adds-peaq-ids-and-role-based-access-control-to-its-network) - "peaq IDs integration."

[^84]: [peaq Blog](https://www.peaq.network/blog/staex-adds-peaq-ids-and-role-based-access-control-to-its-network) - "Role-Based Access Control."

[^85]: [peaq Q1 2024 Review](https://www.peaq.xyz/blog/depin-digest-q1-2024-peaqosystem-review) - "Second milestone in DePIN Grant Program."

[^86]: [peaq Blog](https://www.peaq.network/blog/staex-adds-peaq-ids-and-role-based-access-control-to-its-network) - CLI tools for builders.

[^87]: [DFINITY Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Vehicle Tracking System based on ICP."

[^88]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Rust SDK implementation."

[^89]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "k256 crypto library."

[^90]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoke-vehicles/32723) - "Canister executes monthly."

[^91]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Unblock vehicles after invoice is paid."

[^92]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - ICP advantages listed.

[^93]: [Staex Blog](https://staex.io/blog/partnership-lisk) - RISE project description.

[^94]: [Staex Blog](https://staex.io/blog/partnership-lisk) - "Joined Lisk Accelerator in August 2023."

[^95]: [Unmanned Airspace](https://www.unmannedairspace.info/latest-news-and-information/staex-and-eva-to-build-first-web3-operated-drone-delivery-corridor-in-berlin/) - "Cryptographic hash on the Lisk blockchain."

[^96]: [Staex Blog](https://staex.io/blog/partnership-lisk) - "Journey toward Lisk mainnet integration."

[^97]: [Staex Post](https://www.staex.io/post/staex-solana-superteam-germany-partnership) - July 2025 Solana partnership.

[^98]: [Staex Post](https://www.staex.io/post/staex-solana-superteam-germany-partnership) - "Leveraging Solana for real-time data sharing."

[^99]: [Staex Post](https://www.staex.io/post/staex-solana-superteam-germany-partnership) - "Decentralized infrastructure in Germany."

[^100]: [Messari U2U Report](https://messari.io/report/state-of-u2u-q3-2025) - U2U integration with Staex.

[^101]: [Messari](https://messari.io/report/state-of-u2u-q3-2025) - "Peer-to-peer communication for IoT."

[^102]: [Messari](https://messari.io/report/state-of-u2u-q3-2025) - "Cloud-free, verifiable data exchange."

[^103]: [Messari](https://messari.io/report/state-of-u2u-q3-2025) - "DeFi, RWA, and DePIN verticals."

[^104]: [Linera News](https://linera.io/news/linera-x-staex) - Linera partnership announcement.

[^105]: [Linera News](https://linera.io/news/linera-x-staex) - "10 city buses in North Rhine-Westphalia."

[^106]: [Linera News](https://linera.io/news/linera-x-staex) - "Up to one million transactions per day."

[^107]: [Linera News](https://linera.io/news/linera-x-staex) - "Performance metrics, location data, diagnostics."

[^108]: [Linera News](https://linera.io/news/linera-x-staex) - "Production deployment across 100+ buses."

[^109]: [Linera News](https://linera.io/news/linera-x-staex) - Paksy Plackis-Cheng quote.

[^110]: [Staex NEXA](https://www.staex.io/nexa) - "Machine-to-machine economy demonstration."

[^111]: [Staex NEXA](https://www.staex.io/nexa) - "Led by Staex."

[^112]: [Unmanned Airspace](https://www.unmannedairspace.info/latest-news-and-information/staex-and-eva-to-build-first-web3-operated-drone-delivery-corridor-in-berlin/) - "May 21, 2024 at Marienpark."

[^113]: [Staex NEXA](https://www.staex.io/nexa) - Marienpark location.

[^114]: [Tower Exchange](https://www.towerxchange.com/article/2dcq12ehr996dmaszte68/monetising-towers-as-drone-delivery-platforms) - "Secretary of State Dr. Fissher."

[^115]: [Staex NEXA](https://www.staex.io/nexa) - Nine consortium partners.

[^116]: [Vantage Towers](https://www.vantagetowers.com/en) - "84,600 sites across Europe."

[^117]: [Staex NEXA](https://www.staex.io/nexa) - paymenttools (REWE Group).

[^118]: [Staex NEXA](https://www.staex.io/nexa) - Berlin Partner involvement.

[^119]: [Chirp Medium](https://chirpiot.medium.com/chirp-teams-up-with-staex-and-joins-nexa-consortium-to-lead-web3-innovations-in-germany-82379b3de2b1) - Chirp joins NEXA.

[^120]: [Lisk Year Recap](https://lisk.com/blog/posts/2024-recap/) - Lisk NEXA participation.

[^121]: [Staex NEXA](https://www.staex.io/nexa) - ICP partnership in NEXA.

[^122]: [Germandrones](https://www.germandrones.com/en/news/newsdetail/detail/germandrones-und-staex-gehen-strategische-partnerschaft-ein) - Strategic partnership.

[^123]: [Staex NEXA](https://www.staex.io/nexa) - The Drivery as partner.

[^124]: [Staex NEXA](https://www.staex.io/nexa) - Marienpark venue.

[^125]: [Staex Drone Corridor](https://staex.io/web3-drone-corridor) - "First in the world Web3-operated drone corridor."

[^126]: [Unmanned Airspace](https://www.unmannedairspace.info/latest-news-and-information/staex-and-eva-to-build-first-web3-operated-drone-delivery-corridor-in-berlin/) - Video purchase and blockchain verification.

[^127]: [Berlin Deep Tech Winners](https://www.berlin.de/deeptech/en/blog/artikel.1466383.en.php) - Award in Web3 category.

[^128]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1451680.en.php) - "100 applicants, 15 finalists."

[^129]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/deep-tech-award/) - "€10,000 prize."

[^130]: [Staex MINT](https://www.staex.io/mint) - MINT project overview.

[^131]: [Staex MINT](https://www.staex.io/mint) - "MIBO's 150-busses fleet."

[^132]: [Staex MINT](https://www.staex.io/mint) - "Solana's blockchain infrastructure."

[^133]: [Staex MINT](https://www.staex.io/mint) - "U2U's blockchain platform."

[^134]: [Staex MINT](https://www.staex.io/mint) - "DIMO's platform for AI modeling."

[^135]: [Staex MINT](https://www.staex.io/mint) - "IRIS' components for passenger counting."

[^136]: [Staex MINT](https://www.staex.io/mint) - Technical implementation details.

[^137]: [Staex PANTER](https://www.staex.io/panter) - PANTER platform description.

[^138]: [Staex PANTER](https://www.staex.io/panter) - "Real-time data capture."

[^139]: [Staex PANTER](https://www.staex.io/panter) - "Machine-to-machine transactions."

[^140]: [Staex PANTER](https://www.staex.io/panter) - "Automated invoicing, auditing, certification."

[^141]: [Staex PANTER](https://www.staex.io/panter) - "New business models and revenue streams."

[^142]: [TechBullion](https://techbullion.com/chirp-partners-with-staex-to-deploy-iot-connectivity-across-200000-municipal-buses/) - Chirp partnership scale.

[^143]: [TechBullion](https://techbullion.com/chirp-partners-with-staex-to-deploy-iot-connectivity-across-200000-municipal-buses/) - "Up to 200,000 municipal buses."

[^144]: [TechBullion](https://techbullion.com/chirp-partners-with-staex-to-deploy-iot-connectivity-across-200000-municipal-buses/) - "Up to 12 IoT devices per vehicle."

[^145]: [TechBullion](https://techbullion.com/chirp-partners-with-staex-to-deploy-iot-connectivity-across-200000-municipal-buses/) - Data types collected.

[^146]: [Chirp Token](https://chirptoken.io/) - Chirp network statistics.

[^147]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - VTS capabilities.

[^148]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Trusted, transparent infrastructure."

[^149]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Decentralized telemetry signing."

[^150]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "On-chain contract agreements."

[^151]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Automated invoice generation."

[^152]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Remote vehicle functionality control."

[^153]: [VTS Forum](https://forum.dfinity.org/t/staex-vts-infrastructure-to-track-and-invoice-vehicles/32723) - "Proof of concept complete."

[^154]: [Vantage Towers](https://www.vantagetowers.com/en) - Company overview.

[^155]: [Vantage Towers Wikipedia](https://en.wikipedia.org/wiki/Vantage_Towers) - "84,600 sites as of January 2024."

[^156]: [Vantage Towers](https://www.vantagetowers.com/en) - Key European markets.

[^157]: [Tower Exchange](https://www.towerxchange.com/article/2dcq12ehr996dmaszte68/monetising-towers-as-drone-delivery-platforms) - "Towers as drone vertiports."

[^158]: [Staex Blog](https://staex.io/blog/partnership-vantagetowers-staex-eva) - "Securely collect and verify telemetry."

[^159]: [Tower Exchange](https://www.towerxchange.com/article/2dcq12ehr996dmaszte68/monetising-towers-as-drone-delivery-platforms) - "Infrastructure for commercial drone operations."

[^160]: [Staex Blog](https://staex.io/blog/partnership-staex-taa) - TAA partnership announcement.

[^161]: [Staex Blog](https://staex.io/blog/partnership-staex-taa) - "Two years of successful collaboration."

[^162]: [Germandrones](https://www.germandrones.com/en/news/newsdetail/detail/germandrones-und-staex-gehen-strategische-partnerschaft-ein) - Strategic partnership.

[^163]: [Staex NEXA](https://www.staex.io/nexa) - Drone hardware provider.

[^164]: [Unmanned Airspace](https://www.unmannedairspace.info/latest-news-and-information/staex-and-eva-to-build-first-web3-operated-drone-delivery-corridor-in-berlin/) - "Songbird drone filmed the event."

[^165]: [Staex Official](https://staex.io) - Target verticals.

[^166]: [Staex Official](https://staex.io) - "Autonomous mobility."

[^167]: [Staex Official](https://staex.io) - "Industrial networks."

[^168]: [Staex Official](https://staex.io) - "Public transport."

[^169]: [Staex Official](https://staex.io) - "Logistics."

[^170]: [Staex Official](https://staex.io) - "Energy fleets."

[^171]: [Staex Official](https://staex.io) - "Physical AI systems."

[^172]: [Staex NEXA](https://www.staex.io/nexa) - European focus through NEXA and partnerships.

[^173]: [Staex Official](https://staex.io) - "Bus2, UnB partnership for Brasilia demo October 2025."

[^174]: Blockchain ecosystem partnerships - Global reach through decentralized networks.

[^175]: [Chainwire](https://chainwire.org/2024/01/25/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin/) - DePIN market context.

[^176]: [Staex Official](https://staex.io) - Cloud provider dependency issues.

[^177]: [Chainwire](https://chainwire.org/2024/01/25/staex-joins-peaq-as-a-networking-and-indexing-layer-for-depin/) - 75 billion devices projection.

[^178]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - Web2-Web3 bridge positioning.

[^179]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - Deutsche Telekom heritage.

[^180]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - Zero-trust architecture.

[^181]: [Staex Blog](https://staex.io/blog/partnership-vantagetowers-staex-eva) - B2B enterprise focus.

[^182]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - Legacy protocol encryption.

[^183]: Multiple blockchain integrations documented across peaq, ICP, Lisk, Solana, U2U, Linera partnerships.

[^184]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - "Patented cutting-edge technology."

[^185]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1472032.en.php) - Multi-hop architecture.

[^186]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - 5x lower footprint.

[^187]: [EU-Startups](https://www.eu-startups.com/2022/04/berlin-based-staex-raises-e1-65-million-for-its-unique-and-sustainable-web3-stack-for-iot/) - T-Labs validation.

[^188]: [Berlin Deep Tech](https://www.berlin.de/deeptech/en/blog/artikel.1451680.en.php) - 100 applicants for award.

[^189]: Multiple project deployments - NEXA, Vantage Towers, MIBO.

[^190]: Enterprise partnerships with Vantage Towers, REWE (paymenttools).

[^191]: Multi-blockchain strategy across six+ chains.

[^192]: [Paris Blockchain Week](https://tickets.parisblockchainweek.com/2025/speaker/1651689/alexandra-mikityuk) - PhD leadership credentials.

[^193]: Pre-Series A stage with €1.65M confirmed funding.

[^194]: No token announced as of December 2025.

[^195]: Enterprise B2B sales cycle considerations.

[^196]: DePIN market competitive landscape.

[^197]: Execution risk in scaling pilots.

[^198]: Series A as validation milestone.

[^199]: Potential token launch consideration.

[^200]: MINT production scaling target.

[^201]: [Linera News](https://linera.io/news/linera-x-staex) - 100+ bus deployment plan.

[^202]: [TechBullion](https://techbullion.com/chirp-partners-with-staex-to-deploy-iot-connectivity-across-200000-municipal-buses/) - 200,000 bus target.

[^203]: [Staex Official](https://staex.io) - Brasilia October 2025 demo.

[^204]: Social media analysis - Early community stage.

[^205]: [Twitter @Staex_io](https://twitter.com/Staex_io) - Official account.

[^206]: Community observation - Discord <100 members.

[^207]: [LinkedIn](https://linkedin.com/company/staex) - Company page.

[^208]: [GitHub staex-io](https://github.com/staex-io) - Open source projects.

[^209]: [Staex Official](https://staex.io) - Key messaging themes.

[^210]: [Staex Official](https://staex.io) - "Verifiable real-world data signals."

[^211]: [Staex Official](https://staex.io) - Machine economy messaging.

[^212]: [Staex Official](https://staex.io) - "Tamper-proof logging."

[^213]: [Staex Official](https://staex.io) - "No cloud lock-ins."

[^214]: German regulatory environment analysis.

[^215]: [Tower Exchange](https://www.towerxchange.com/article/2dcq12ehr996dmaszte68/monetising-towers-as-drone-delivery-platforms) - Secretary of State inauguration.

[^216]: [Staex NEXA](https://www.staex.io/nexa) - "Lighthouse project by Berlin Partner."

[^217]: EU digital sovereignty alignment.

[^218]: Zero-trust architecture aligns with GDPR principles.

[^219]: [Tower Exchange](https://www.towerxchange.com/article/2dcq12ehr996dmaszte68/monetising-towers-as-drone-delivery-platforms) - "Discussions with German authorities."

[^220]: MiCA compliance consideration if token launched.

[^221]: Source methodology for seed-stage company.

[^222]: B2B infrastructure - no TVL metrics.

[^223]: No token data available.

[^224]: MN Capital amount undisclosed.

[^225]: Forward-looking targets (200,000 buses).

[^226]: Strategic positioning analysis.

[^227]: Execution capability demonstrated through NEXA.

[^228]: Differentiator summary.

[^229]: Investor monitoring recommendations.

[^230]: TAM thesis based on 75 billion device projection.
